...
export function validateStudySchedule(input: {
  ...
  const knownQuestionMap = new Map<string, InterviewQuestion>();
  ...
  const scheduledQuestionCounts = new Map<string, number>();
  ...
  const coveredByScheduled = new Set<string>();
