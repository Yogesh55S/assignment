import type { Requirement, InterviewQuestion, ScheduleDay } from "../../types/kit.js";
import type {
  QuestionSchedulingScore,
...
import {
  ...
  DEFAULT_EMPTY_DAY_MINUTES,
  PRIORITY_WEIGHT_MUST,
  PRIORITY_WEIGHT_NICE,
  PRIORITY_WEIGHT_NONE,
 ...
 */
export function calculateQuestionSchedulingScores(
  requirements: Requirement[],
  questions: InterviewQuestion[]
): QuestionSchedulingScore[] {
  // Map requirement ID to its priority for fast lookup
  const reqMap = new Map<string, "must" | "nice">();
  ...
  const uniqueQuestions: InterviewQuestion[] = [];
  ...
  const scores: QuestionSchedulingScore[] = [];
...
export function allocateStudySchedule(input: {
  ...
  if (
    ...
    throw new Error(
      `Invalid daysAvailable: ${daysAvailable}. Must be an integer between ${MIN_SCHEDULE_DAYS} and ${MAX_SCHEDULE_DAYS}.`
    );
  }
  ...
  const scoreMap = new Map<string, QuestionSchedulingScore>();
  ...
  for (const day of days) {
    if (day.question_ids.length === 0) {
      ...
      for (const qId of day.question_ids) {
        ...
        const difficulty = (q?.difficulty as 1 | 2 | 3) ?? 1;
  ...
  const mustRequirementIdsScheduled: string[] = [];
